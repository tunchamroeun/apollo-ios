...
struct ExportsFileGenerator {
  ...
  static func generate(
    ...
    case .swiftPackageManager:
        filePath = pathURL.appendingPathComponent("Sources").appendingPathComponent("Internal").appendingPathComponent("Exports.swift").path
      rendered = ExportsTemplate(
        config: config
      ).render()
