...
struct SelectionSetTemplate {
  ...
  private func ParentTypeTemplate(_ type: GraphQLCompositeType) -> String {
    ...
    \(type.schemaTypesNamespace).\(type.name.firstUppercased) }
    """
  }
...
struct MutableSelectionSetTemplate {
  ...
  func BodyTemplate(_ selectionSet: IR.SelectionSet) -> TemplateString {
    ...
    let scope = selectionSet.typeInfo.scope
    return """
      \(DataFieldAndInitializerTemplate(selections))
  ...
  private func DataFieldAndInitializerTemplate(_ selections: IR.SelectionSet.Selections) -> String {
    """
    public \(isMutable ? "var" : "let") __data: DataDict
    public init(data: DataDict) { __data = data }

    public init(\(InitializerParametersTemplate(selections))) {
      __data = DataDict([\(DataDictInitializerTemplate(selections))], variables: [:])
    }
    """
  }
  ...
  private func InitializerParametersTemplate(
    ...
    TemplateString(
      """
      \(ifLet: selections.direct?.fields.values, {
          "\(list: $0.map({"\($0.responseKey.firstLowercased.asFieldAccessorPropertyName): \(renderTypeName(for: $0, forceOptional: false))"}))"
      })
      """
    )
  }
    
    private func DataDictInitializerTemplate(
      _ selections: IR.SelectionSet.Selections
    ) -> TemplateString {
      TemplateString(
        """
        \(ifLet: selections.direct?.fields.values, {
            "\(list: $0.map({"\"\($0.responseKey.firstLowercased.asFieldAccessorPropertyName)\": \($0.responseKey.firstLowercased.asFieldAccessorPropertyName)"}))"
        })
        """
      )
    }

  private func renderTypeName(for field: IR.Field, forceOptional: Bool = false) -> String {
    let fieldName: String
    switch field {
    case let scalarField as IR.ScalarField:
      fieldName = scalarField.type.rendered(as: .selectionSetField(), config: config.config)
  ...
  private func FragmentAccessorsTemplate(
    ...
    guard !(selections.direct?.fragments.isEmpty ?? true) || !selections.merged.fragments.isEmpty
    else {
      return ""
    }
  ...
  private func ChildTypeCaseSelectionSets(_ selections: IR.SelectionSet.Selections)
    -> TemplateString
  {
    """
    \(ifLet: selections.direct?.inlineFragments.values, {
